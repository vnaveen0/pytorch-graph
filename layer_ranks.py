...
class SummaryGraph(object):
    ...
    def __init__(self, model, dummy_input, apply_scope_name_workarounds=True):
        ...
        with torch.onnx.set_training(model_clone, False):
            ...
            # For pytorch 1.4
            # trace, _ = jit._get_trace_graph(model_clone, dummy_input)
            # traverse nodes with trace.nodes()
            ...
            # For pytorch 1.3 -- trace.graph.nodes()
    ...
    def layers_topological_order(self, recurrent=False):
        ...
        for idx, key in zip(new_order,ret2):
            ...
            pred_rank_list_str = ','.join([str(i) for i in pred_rank_list])

            succ_list = ranked_ops[key].adj_entry.successors
